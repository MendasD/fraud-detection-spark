...
class FraudDetector:
    ...
    def _apply_rule_based_detection(self, df):
        ...
        VERSION STREAMING - Règles sans dépendances temporelles complexes.
        ...
        """
        logger.info("Application des règles de détection (VERSION STREAMING)")
        
        # Initialiser le score à 0
        df = df.withColumn('fraud_score', lit(0.0))
        
        # Règle 1: Montant très élevé (+40 points)
        df = df.withColumn(
            'fraud_score',
            when(col('is_very_high_amount') == 1, col('fraud_score') + 40)
            .when(col('is_high_amount') == 1, col('fraud_score') + 25)
        ...
        # Règle 3: Montant rond + élevé (souvent suspect) (+15 points)
        df = df.withColumn(
            'fraud_score',
            when(
                (col('is_round_amount') == 1) & (col('amount') > 200),
                col('fraud_score') + 15
            ).otherwise(col('fraud_score'))
        )
        
        # Règle 4: Marchand à haut risque + montant élevé (+20 points)
        df = df.withColumn(
            'fraud_score',
            when(
        ...
        # Règle 5: Transaction en ligne + très gros montant (+15 points)
        df = df.withColumn(
            'fraud_score',
            when(
                (col('is_online') == True) & (col('amount') > 500),
                col('fraud_score') + 15
            ).otherwise(col('fraud_score'))
        )
        
        # Règle 6: Week-end + heure inhabituelle + montant élevé (+20 points)
        df = df.withColumn(
            'fraud_score',
            when(
                (col('is_weekend') == 1) & 
                (col('is_unusual_hour') == 1) & 
                (col('amount') > 200),
                col('fraud_score') + 20
            ).otherwise(col('fraud_score'))
        )
